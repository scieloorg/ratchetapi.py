...
from ratchetapi import httpbroker, exceptions
import doubles
...
class TranslateExceptionsTests(unittest.TestCase):
    ...
    def test_from_ConnectionError_to_ConnectionError(self):
        ...
        to ratchetapi.exceptions.ConnectionError
        """
        import requests
    ...
    def test_from_HTTPError_to_HTTPError(self):
        ...
        to ratchetapi.exceptions.HTTPError
        """
        import requests
    ...
    def test_from_Timeout_to_Timeout(self):
        ...
        to ratchetapi.exceptions.Timeout
        """
        import requests
...
class GetFunctionTests(mocker.MockerTestCase):
    ...
    def test_user_agent_is_properly_set(self):
        """
        By properly I mean: ratchetapi/:version, e.g.
        ratchetapi/0.4
        """
        import requests
        ...
        mock_requests_get(
            'http://ratchet.scielo.org/api/v1/journals/70/',
            headers=mocker.MATCH(lambda x: x['User-Agent'].startswith('ratchetapi/')),
            params=None
        )
        ...
        self.assertEqual(
            httpbroker.get('http://ratchet.scielo.org/api/v1/',
    ...
    def test_resource_id_makes_endpoint_mandatory(self):
        self.assertRaises(
            ValueError,
            lambda: httpbroker.get('http://ratchet.scielo.org/api/v1/', resource_id='70')
        )
    ...
    def test_https_turns_off_ca_cert_verification(self):
        ...
        mock_requests_get = self.mocker.mock()
        mock_requests_get('https://ratchet.scielo.org/api/v1/journals/70/',
                          headers=mocker.ANY,
                          params=None,
                          verify=False)
        self.mocker.result(mock_response)
        ...
        self.assertEqual(
            httpbroker.get('https://ratchet.scielo.org/api/v1/',
                endpoint='journals', resource_id='70'),
            {'title': 'foo'}
        )
...
class PostFunctionTests(mocker.MockerTestCase):
    ...
    def test_user_agent_is_properly_set(self):
        ...
        self.mocker.result({'location': 'http://ratchet.scielo.org/api/v1/journals/4/'})
        self.mocker.count(2)
        ...
        mock_requests_post(
            url='http://ratchet.scielo.org/api/v1/journals/',
            headers=mocker.MATCH(lambda x: x['User-Agent'].startswith('ratchetapi/')),
            data='{"title": "foo"}'
        )
        self.mocker.result(mock_response)
        ...
        self.assertEqual(
            httpbroker.post('http://ratchet.scielo.org/api/v1/',
                endpoint='journals', data='{"title": "foo"}'),
            'http://ratchet.scielo.org/api/v1/journals/4/'
        )
    ...
    def test_content_type_is_properly_set(self):
        ...
        mock_requests_post = self.mocker.mock()
        mock_requests_post(url='http://ratchet.scielo.org/api/v1/journals/',
    ...
    def test_unexpected_status_code_raises_APIError(self):
        ...
        self.assertRaises(exceptions.APIError,
                          lambda: httpbroker.post('http://ratchet.scielo.org/api/v1/',
                                                  endpoint='journals',
                                                  data='{"title": "foo"}')
        )
...
class MakeFullUrlFunctionTests(unittest.TestCase):
    ...
    def test_missing_trailing_slash(self):
        path_segments = ['http://ratchet.scielo.org', 'api', 'v1', 'journals']
        self.assertEqual(httpbroker._make_full_url(*path_segments),
            'http://ratchet.scielo.org/api/v1/journals/')

    def test_missing_scheme(self):
        path_segments = ['ratchet.scielo.org', 'api', 'v1', 'journals']
        self.assertEqual(httpbroker._make_full_url(*path_segments),
            'http://ratchet.scielo.org/api/v1/journals/')

    def test_https(self):
        path_segments = ['https://ratchet.scielo.org', 'api', 'v1', 'journals']
        self.assertEqual(httpbroker._make_full_url(*path_segments),
            'https://ratchet.scielo.org/api/v1/journals/')
